...
CREATE TABLE aura.aggregated_user_hourly_data_new
(
    `user_uid` String,
    `tkn_uuid` UUID,
    `timestamp` DateTime,
...
ORDER BY (user_uid, tkn_uuid, timestamp, rpc_method, chain, request_type, is_mainnet)
...
INSERT INTO aura.aggregated_user_hourly_data_new
SELECT
    user_uid,
    tkn_uuid,
    timestamp,
...
RENAME TABLE aura.aggregated_user_hourly_data_new TO aura.aggregated_user_hourly_data;
